...
4. Create `README.md` and a discoverable TODO ledger, or ask the agent to create them and seeding agent with the idea, constraints, and existing sources.
...
* IEF is designed to work with agentic CLIs and IDE extensions such as Codex, Claude Code, OpenCode, and GitHub Copilot.
...
The live ledger is deliberately bounded. By default, an owner removes a completed item only after it is at least 30 days old and no longer among the 10 most recent `DONE` items in that ledger. Before removal, the owner promotes reusable decisions, instructions, role learnings and unresolved follow-ups to their proper long-term surfaces. Open, blocked, referenced and unpushed work is never pruned. The old item is deleted rather than copied to an archive ledger because its exact execution log remains available in Git; cleanup is committed separately from product work. See [Ledger retention and distillation](AGENTS.md#ledger-retention-and-distillation) for the full contract.
